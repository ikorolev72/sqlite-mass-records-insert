...
while(<IN>){
  chomp();
  my $record=parse_data_image_details( $_ );
  if( !$record ) {
...
This script insert data from file into tables image_details
...
Where:
	...
	--help - this help
Sample:	${0} --filename=Image_details_table.txt
");
	exit (1);
}
